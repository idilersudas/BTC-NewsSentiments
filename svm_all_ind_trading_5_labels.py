...
sentiment_file_path = 'C/Users/licop/Desktop/MSc_DataScience/Interdisciplinary_Group_Project/ModelDevelopment/data/Daily_Sentiment_Analysis_VADER.csv'
...
trading_volume_file_path = 'Desktop/MSc_DataScience/Interdisciplinary_Group_Project/ModelDevelopment/data/trading_volume_with_labels.csv'
...
# Apply a threshold for making confident decisions
confidence_threshold = 0.7  # Example threshold

# Function to make trading decisions based on a threshold
def make_trading_decisions(probs, threshold):
    decisions = np.zeros(probs.shape[0], dtype=int)
    for i in range(probs.shape[0]):
        if np.max(probs[i]) > threshold:
            decisions[i] = np.argmax(probs[i])
        else:
            decisions[i] = np.random.choice([0, 1, 2, 3, 4])  # Random decision if confidence is low
    return decisions

# Apply the threshold to get trading decisions
y_pred_threshold = make_trading_decisions(y_pred_prob, confidence_threshold)

# Ensure trades are integers
model_trades = np.tile(y_pred_threshold, (num_people, 1))

# Simulate random trading
np.random.seed(42)
random_trades = np.random.choice([0, 1, 2, 3, 4], size=(num_people, num_trades))
